...
from my_tutor.schemes import AddUserRequest, AddUserResponse
...
async def add_user(user_data: AddUserRequest, session: AsyncSession = Depends(get_db_session)) -> AddUserResponse:
    try:
        async with session.begin():
            return await user_repository.add_user(session, user_data)
